...
use crate::{store::ItemList, element::Element, scheduler::Scheduler};
use crate::controller::ControllerMessage;
use crate::{Message, exit};
...
impl View {
    ...
    pub fn new(sched: Rc<Scheduler>) -> Option<View> {
        ...
        let new_todo = Element::qs(".new_todo")?;

        Some(View {
            sched: RefCell::new(sched),
            todo_list, todo_item_counter, clear_completed, main, toggle_all, new_todo, callbacks: Vec::new(),
        })
    }
    ...
    pub fn init(&mut self) {
        ...
        let set_page = Closure::wrap(Box::new(move || {
            if let Some(location) = document.location() {
                if let Ok(hash) = location.hash() {
                    if let Ok(sched) = &(sched.try_borrow()) {
        ...
        window_et.add_event_listener_with_callback("hashchange", set_page.as_ref().unchecked_ref()).unwrap();
    ...
    /// Used by scheduler to convert `ViewMessage` into a function call on the `View`
    ...
    /// Populate the todo list of items
    ...
    fn set_items_left(&mut self, items_left: usize) {
        self.todo_item_counter.set_inner_html(Template::item_counter(items_left));
    }

    /// Set the number of 'items left' display
    ...
    fn update_filter_buttons(&self, route: &str) {
        ...
        // let selector = format!("[href=\"{}\"", route);
    ...
    fn bind_toggle_all(&mut self) {
        let sched = self.sched.clone();
        self.toggle_all.add_event_listener("click", move |event: web_sys::Event| {
            if let Some(target) = event.target() {
                if let Some(input_el) = wasm_bindgen::JsCast::dyn_ref::<web_sys::HtmlInputElement>(&target) {
                    if let Ok(sched) = &(sched.try_borrow_mut()) {
                        sched.add_message(Message::Controller(ControllerMessage::ToggleAll(input_el.checked())));
                    }
                }
            }
        });
    ...
    fn bind_remove_item(&mut self) {
        let sched = self.sched.clone();
        self.todo_list.delegate(".destroy", "click", 
    ...
    fn bind_toggle_item(&mut self) {
        let sched = self.sched.clone();
        self.todo_list.delegate(".toggle", "click", 
            move |e: web_sys::Event| {
                if let Some(target) = e.target() {
                    ...
                    if let Some(item_id) = item_id(el) {
                        if let Ok(sched) = &(sched.try_borrow_mut()) {
                            sched.add_message(Message::Controller(ControllerMessage::ToggleItem(item_id, checked)))
                        }
                    }
                }
            }, false);
    ...
    fn bind_edit_item_save(&mut self) {
        ...
        self.todo_list.delegate("li .edit", "blur", move |e: web_sys::Event| {
            if let Some(target) = e.target() {
        ...
        self.todo_list.delegate("li .edit", "keypress", |e: web_sys::Event| {
    ...
    fn bind_edit_item_cancel(&mut self) {
        let sched = self.sched.clone();
        self.todo_list.delegate("li .edit", "keyup", move |e: web_sys::Event| {
            if let Some(key_e) = wasm_bindgen::JsCast::dyn_ref::<web_sys::KeyboardEvent>(&e) {
...
impl Drop for View {
    fn drop(&mut self) {
        ...
        let callbacks: Vec<(web_sys::EventTarget, String, Closure<dyn FnMut()>)> = self.callbacks.drain(..).collect();
        for callback in callbacks {
            callback.0.remove_event_listener_with_callback(
                callback.1.as_str(), &callback.2.as_ref().unchecked_ref(),).unwrap()
        }
    }
}