...
/// Messages that represent the methods to be called on the Conteroller
...
impl Controller {
    ...
    /// Used by `Scheduler` to convert a `ControllerMessage  into function call on `Controller`
    pub fn call(&mut self, method_name: ControllerMessage) {
        ...
        fn remove_completed_items(&mut self) {
            self.store.remove(crate::store::ItemQuery::Completed { completed: true });
            self._filter(true);
        }
