...
impl From<web_sys::EventTarget> for Element {
    fn from(el: web_sys::EventTarget) -> Self {
...
impl Element {
    ...
    pub fn create_element(tag: &str) -> Option<Self> {
    ...
    where T: 'static + FnMut(web_sys::Event), {
        let cb = Closure::wrap(Box::new(handler) as Box<dyn FnMut(_)>);
        if let Some(el) = self.el.take() {
            let el_et: EventTarget = el.into();
            el_et.add_event_listener_with_callback(event_name, cb.as_ref().unchecked_ref()).unwrap();
            cb.forget();
    ...
    /// Delegate an event to aslector
    pub fn delegate<T>(&mut self, selector: &'static str, event: &str, mut handler: T, use_capture: bool)
    where T: 'static + FnMut(web_sys::Event) -> (), {
        ...
        if let Some(dyn_el) = &el.dyn_ref::<EventTarget>() {
            if let Some(window) = web_sys::window() {
                if let Some(document) = window.document() {
                    // TODO element selector to the target element
                    let tg_el = document;

                    let cb = Closure::wrap(Box::new(move |event: web_sys::Event| {
                        if let Some(target_element) = event.target() {
                            let dyn_target_el: Option<&web_sys::Node> = wasm_bindgen::JsCast::dyn_ref(&target_element);
    ...
    /// Gets the text content of the 1`self.el` element
    ...
    /// Appends child element to `self.el` element
    ...
    /// Remove a class list item from the element
    /// 
    /// ```
    ...
    /// Gets the checkout state for the element in `self.el` (The element must be an input)
